...
A compressed file with the binaries is created in `<Terrarheimr>\Packages`ready for upload to ThunderStore.
...
A Unity project is included in this repository under `<Terrarheimr>\JotunnModUnity`.
...
4. Compile the project. This copies all assemblies into `<Terrarheimr>\JotunnModUnity\Assets\Assemblies`. Don't open Unity yet before this step, it will remove assembly references.
...
1. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `<Terrarheimr>\JotunnModUnity\Assets\Assemblies`. <br />
